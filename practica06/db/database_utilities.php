<?php
	...
	//Funcion que permite agregar un nuevo usuario a la tabla User de la base de datos.
	function addUser($email,$pass,$status,$user_type_id){
		global $pdo;
		$stmt = $pdo->prepare("INSERT INTO user(email,password,status_id,user_type_id)
					VALUES('$email', '$pass', '$status', '$user_type_id')");
		return $stmt->execute();
	}
	...
	function login($id,$password){
		...
		$sql = $pdo->prepare("SELECT * FROM user where id=:id AND password=:password");
		$sql->bindParam(':id',$id);
